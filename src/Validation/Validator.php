...
/**
 * Validates a value against a schema: "does this value conform?", answered as every way in which it does not.
 *
 * A reflection-free visitor over the concrete {@see Schema} types that reads their public constraint properties and
 * aggregates every violation as an {@see Issue}, which {@see self::validate()} turns into a {@see ValidationResult}.
 * It judges the value exactly as given: it neither converts nor reshapes it, and it creates no PHP objects.
 *
 * **Input is JSON decoded to associative arrays** — `json_decode($json, true)`. A JSON object is a PHP array with
 * string keys; a `stdClass` is not one, and is reported as the wrong type rather than quietly unwrapped. One input
 * shape is the whole point: a payload this accepts is a payload `neos/schematic` can build from, so a value cannot
 * be valid here and unbuildable there.
 *
 * Two things this deliberately does *not* do, because neither is a property of a schema:
 *
 * - *reading a value that is spelled like its type* (`"45"` -> `45`) belongs to whatever knows the value came over
 *   a wire as a string — `Neos\OpenApi\Binding\ScalarNormalizer` does it for path, query, header and cookie
 *   parameters, before handing the value here;
 * - *reshaping a value onto the structure its schema describes* is a producer's business on the way out, and
 *   nobody's on the way in – `Neos\Schematic\Serialization\Projection` does it for a package emitting JSON.
 *
...
final class Validator
{
    ...
    public static function validate(Schema $schema, mixed $value): ValidationResult
    {
        $issues = self::check($schema, $value);
     ...
     * @return list<Issue>
     */
    private static function check(Schema $schema, mixed $value, array $path = []): array
    {
    ...
    private static function validateObject(ObjectSchema $schema, mixed $value, array $path): array
    {
        if (!is_array($value) || ($value !== [] && array_is_list($value))) {
            // an object is an associative array here, a stdClass is not — see the class doc block
